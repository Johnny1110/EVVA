...
// — a flexible researcher / multi-step task runner. Ported 1:1 from
// ref/src/tools/AgentTool/built-in/generalPurposeAgent.ts (SHARED_PREFIX +
// SHARED_GUIDELINES), with "Claude Code" replaced by "evva" for identity.
//
// Like Explore, the prompt is a single hand-written string with no shared
// fragments and no memory injection. PromptContext is accepted for API
// uniformity with the other builders; today it is unused.
