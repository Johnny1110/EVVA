...
func (t *AgentTool) Description() string {
	return "Spawn a sub-agent to handle a focused task in isolation. " +
		...
		"Level=2 is more expensive — only use it when the task genuinely needs deeper thinking. " +
...
func (t *AgentTool) Schema() json.RawMessage {
	return json.RawMessage(`{
		...
		"properties":{
			...
			"level":{"type":"integer","enum":[1,2],"default":1,"description":"Model tier within the parent's provider. 1 = normal (Sonnet / DeepSeek-Flash / equivalent), 2 = big (Opus / DeepSeek-Pro / equivalent). Defaults to 1. Use 2 only when the task genuinely needs deeper reasoning."}
		}
	}`)
}
...
func (t *AgentTool) Execute(ctx context.Context, input json.RawMessage) (tools.Result, error) {
	...
	spawner := t.lookup()
	if spawner == nil {
		// Likely cause: the AGENT tool was reached from a subagent (the
		// agent layer only installs the spawner on root agents).
