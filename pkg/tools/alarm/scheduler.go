...
// defaultJitter returns a uniform random delay in [0, maxJitter) spreading
// recurring fires that share a cron expression.
func defaultJitter() time.Duration {
	return time.Duration(mathrand.Int64N(int64(maxJitter)))
}
...
func (s *Scheduler) Arm(a Alarm) (Alarm, error) {
	...
	if a.CronExpr != "" {
		...
		if a.Recurring && a.Expiry.IsZero() {
...
func (s *Scheduler) Rearm() ([]Fired, error) {
	...
	for _, a := range stored {
		...
		if a.CronExpr != "" && a.Recurring {
			// Recurring cron: drop if expired, else recompute the next match from
			// now (missed fires are skipped, not caught up). One-shot crons fall
			// through to the plain-alarm branches below on their stored FireAt.
			if !a.Expiry.IsZero() && now.After(a.Expiry) {
				continue // expired: drop from store
			}
			if s.cronNext == nil {
...
func (s *Scheduler) armLocked(a Alarm, now time.Time) {
	...
	// Add jitter for cron entries to spread load when they share an expression.
	if a.CronExpr != "" {
		d += s.jitter()
	}
	id := a.ID
...
func (s *Scheduler) fire(id string) {
	...
	switch {
	case a.CronExpr != "" && a.Recurring:
		// Recurring cron: re-arm at the next match unless expired or no longer
		// schedulable. fired.Recurring stays false on any of those exits so the
		// final fire reads as a plain "Cron job fired" with no next time.
		switch {
		case !a.Expiry.IsZero() && now.After(a.Expiry):
			delete(s.alarms, id) // auto-expired
		case s.cronNext == nil:
			delete(s.alarms, id) // cron evaluator gone (cannot happen once armed)
		default:
			next, err := s.cronNext(a.CronExpr, now)
			if err != nil {
				delete(s.alarms, id) // no further activation within the horizon
			} else {
				ar.alarm.FireAt = next
				fired.Recurring = true
				fired.NextFire = next
				d := max(next.Sub(now), 0) + s.jitter()
				ar.timer = time.AfterFunc(d, func() { s.fire(id) })
			}
		}
	default:
		// One-shot alarm or one-shot cron: fire once, then self-remove.
		delete(s.alarms, id)
	}
