...
func doingTasksSection() string {
	return "# Doing tasks\n" +
		...
		" - When implementing a new tool, handler, or non-trivial function, write a test that exercises its core path BEFORE claiming it works. Never ship code that hasn't been verified through a test — a bug found by the user costs far more trust than the minutes it takes to write the test.\n" +
		" - Before calling a third-party library function (e.g. excelize, an AWS SDK, a chart lib), read the actual struct definition and function signature from the library's source in GOPATH/pkg/mod. Do not assume field names, parameter counts, or return types — verify them against the real source. One mismatched struct field silently drops data (e.g. duplicate `json:\"x\"` tags on two struct fields); one wrong function signature wastes a build-and-fix cycle.\n" +
